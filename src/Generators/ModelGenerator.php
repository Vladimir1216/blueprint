...
class ModelGenerator extends AbstractClassGenerator implements Generator
{
    ...
    protected function hiddenColumns(array $columns): array
    {
        return array_keys(array_filter(
            array_map(
                fn (Column $column) => in_array('hidden', $column->modifiers()) || in_array($column->name(), ['password', 'remember_token']),
                $columns
            )
        ));
    }
    ...
    protected function addTraits(Model $model): void
    {
        ...
        foreach ($model->traits() as $trait) {
            ...
            $prnt = Str::afterLast($trait, '\\');
            if (Str::contains($prnt, ' as ')) {
                $prnt = Str::afterLast($prnt, 'as ');
            }
            $this->addTrait($model, $prnt);
        }
    }
    ...
    protected function addInterfaces(Model $model): void
    {
        foreach ($model->interfaces() as $interface) {
            ...
            $prnt = Str::afterLast($interface, '\\');
            if (Str::contains($prnt, ' as ')) {
                $prnt = Str::afterLast($prnt, 'as ');
            }
            $this->addInterface($model, $prnt);
        }
    }
